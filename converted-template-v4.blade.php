{{-- Enhanced Malaysian Wedding Template v3 with Fullscreen Autoplay Video --}}
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    ...
    <style>
        ...
        * {
        ...
        /* Small Mobile Landscape - Show Vertical Video */
        ...
        .fallback-play-button {
            ...
            height: 100px;
            border-radius: 50%;
            background: rgba(200, 161, 101, 0.95);
...
<body class="invitation-active">
    ...
    <div class="video-section" id="videoSection">
        ...
        <div class="video-loading active" id="videoLoading">
    ...
    <script>
        ...
        function detectCurrentVideo() {
            ...
            // Check which video is currently visible based on CSS display
            const horizontalStyle = window.getComputedStyle(videoHorizontal);
            const verticalStyle = window.getComputedStyle(videoVertical);
            
            console.log('Horizontal video display:', horizontalStyle.display);
            console.log('Vertical video display:', verticalStyle.display);
            
            if (horizontalStyle.display !== 'none') {
                console.log('Using horizontal video');
                return videoHorizontal;
            } else if (verticalStyle.display !== 'none') {
                console.log('Using vertical video');
                return videoVertical;
            }
            
            console.log('Falling back to horizontal video');
            return videoHorizontal; // fallback
        }

        function setupVideoListeners(video) {
            if (!video) {
                console.error('No video element provided to setupVideoListeners');
                return;
            }
            
            console.log('Setting up listeners for video:', video.id);
            console.log('Video src:', video.querySelector('source')?.src || 'No source found');
            console.log('Video poster:', video.poster || 'No poster');
            
            // Test if this is a fallback URL (meaning regex processing failed)
            const src = video.querySelector('source')?.src || '';
            if (src.includes('%7B%7B') || src.includes('{{')) {
                console.error('⚠️ BLADE PROCESSING FAILED - Video src contains unprocessed Blade syntax!');
                console.error('This means the template regex patterns are not working correctly.');
        ...
        function switchVideo() {
            ...
            // Detect which video should be active
            const newVideo = detectCurrentVideo();
            ...
            if (newVideo !== currentVideo) {
                ...
                // Setup listeners for new video
                setupVideoListeners(currentVideo);
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            currentVideo = detectCurrentVideo();
            console.log('Initial current video:', currentVideo?.id || 'null');
            ...
            if (currentVideo) {
                ...
                setTimeout(() => {
                    console.log('Checking video autoplay status...');
                    console.log('Video paused:', currentVideo.paused);
                    console.log('Video started:', videoStarted);
                    console.log('Autoplay failed:', autoplayFailed);
                    
                    if (currentVideo && currentVideo.paused && !videoStarted) {
                        console.log('Autoplay failed, showing fallback button');
                        autoplayFailed = true;
                        if (fallbackPlayBtn) {
                            fallbackPlayBtn.style.display = 'flex';
                            console.log('Fallback button shown');
                        }
                    }
                }, 1500);
            } else {
                console.error('No current video detected');
                if (fallbackPlayBtn) fallbackPlayBtn.style.display = 'flex';
            }
            ...
            window.addEventListener('orientationchange', function() {
                setTimeout(switchVideo, 100); // Small delay to ensure CSS has updated
            });
            ...
            if (invitationOverlay) {
                ...
                invitationOverlay.addEventListener('click', () => {
                    ...
                    setTimeout(() => {
                        ...
                        if (currentVideo && autoplayFailed) {
                            currentVideo.play().then(() => {
